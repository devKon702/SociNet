  ...
  return (
    <div className="flex gap-2 items-start justify-start my-3 text-gray-800">
      <div className="avatar rounded-full overflow-hidden size-10">
        <img src={comment.user.avatarUrl || "unknown-avatar.png"} alt="" />
      </div>
      <div>
        <div className="comment rounded-2xl bg-slate-200 p-2 popup-container">
          <p className="font-bold">{comment.user.name}</p>
          <p>{comment.content}</p>

          {user.id === comment.user.id && (
            <div className="px-2 popup top-1/2 right-0 translate-x-full -translate-y-1/2 flex gap-2">
              <i
